...
public class FileBackedTasksManager implements TaskManager {
    ...
    public TaskManager delegate = Managers.getDefault();
    ...
    public void save() {
        try (Writer writer = new FileWriter(file, StandardCharsets.UTF_8)) {
            ...
            writer.write("\n");
            writer.write(historyToString(InMemoryTaskManager.historyManager));
        ...
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader((file)))) {
            ...
            for (int id : historyFromString(valueHistory)) {
                if (getFileBackedTasksMap().containsKey(id)) {
                    InMemoryTaskManager.historyManager.add(getFileBackedTasksMap().get(id));
                }
            }
        } catch (IOException exception) {
    ...
    public static Task fromString(String value) {
        ...
        if (type.equals(TaskTypes.TASK)) {
            return new Task(title, id, description, status);

        } else if (type.equals(TaskTypes.EPIC)) {
            return new Epic(title, id, description);

        } else {
            return new SubTask(title, id, description, Integer.parseInt(lines[5]), status);
        }
    }
